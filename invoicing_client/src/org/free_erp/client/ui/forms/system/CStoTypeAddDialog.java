 ...
 */
public class CStoTypeAddDialog extends CBaseListDetailedDialog implements ActionListener
{
    public JDataField addField;
    public CStoTypeAddDialog(JDialog parent, IDataSource dataSource)
    {
        super(parent ,dataSource);
        this.setTitle("添加仓库类型");
    ...
    protected CPanel getMainPanel()
    {
        ...
        this.addField = new JDataField("stoType", String.class, dataSource);
        this.addField.setRequired("仓库类型", true);
        this.addField.setMaxLength(30);
        panel.addComponent(this.addField, 70, 5, 100, 20, "仓库类型：", 60);
        return panel;
    }
    ...
    protected boolean isExist()
    {
        ...
        {
            String stoType = (String)this.dataSource.getDataRows().get(i).getColumnValue("stoType");
            if(stoType != null)
            {
                if(stoType.equals(this.addField.getText()))
                {
                    MessageBox.showMessageDialog(this, "“" + stoType +"”已经存在！");
                    return true;
                }
            }
        }
    ...
    public void newDataRow()
    {
		...
		storageType.setCompany(Main.getMainFrame().getCompany());
		IDataRow dataRow = ObjectDataRow.newDataRow(storageType, "id", this.dbSupport);
     ...
     public void createDbSupport()
    {
		dbSupport = new IDbSupport()
        {
			...
			public void delete(Object obj)
            {
                ...
                {
					StorageType po = (StorageType) obj;
					optionSetService.deleteStorageType(po);
					pool.refreshStorageTypes();
				}
                else
                {
			...
			public void save(Object obj)
            {
                ...
                {
					StorageType po = (StorageType) obj;
					optionSetService.saveStorageType(po);
					pool.refreshStorageTypes();
				}
                else
                {
