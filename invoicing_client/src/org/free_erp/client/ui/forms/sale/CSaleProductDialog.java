 ...
 * @author tzl
 */
public class CSaleProductDialog extends CBaseProductSelectDialog implements ActionListener, ValueChangedListener {
    ...
    public CSaleProductDialog(JDialog parent, IDataSource dataSource, Storage storage) {
        super(parent, dataSource, storage);
    }
   public CSaleProductDialog(JDialog parent, IDataSource dataSource) {
        super(parent, dataSource);
    }
    public CSaleProductDialog(JDialog parent, IDataSource dataSource, Storage storage, Customer customer) {
    ...
    protected void initC() {
        ...
        tabbedPane.setBounds(10, 5, 630, 260);
...
//        this.priceField.addItemListener(this);
        priceField.addValueChangedListener(this);
    ...
    private void refreshDatas() {
        ...
        if (obj instanceof Product) {
            Product product = (Product) obj;
//            if(this.dataSource.getBindingValue("price")!=null){
//                this.priceField.setSelectedItem(this.dataSource.getBindingValue("price"));//
//            }else
            if (this.getHistoryPeice() != 0d) {
                this.priceField.setSelectedItem(this.getHistoryPeice());//
            } else {
...
//                s = Double.parseDouble(df.format(s));
                ...
                taxPriceField.setText(String.valueOf(0.00));
...
//    }
    ...
    public void valueChanged(ValueChangedEvent evt) {
        ...
        if (evt.getSource() == this.amountField) {
            double stock = Double.valueOf(stockField.getText());
            double amount = Double.valueOf(amountField.getText());
            if (amount > stock) {
                MessageBox.showErrorDialog(this, "您的库存数量只有" + stock);
                this.amountField.setText("");
                return;
            } else if (amount <= 0) {
                MessageBox.showErrorDialog(this, " 商品数量不能为0或小于0");
                this.amountField.setText("");
                return;
            } else {
                this.refreshAmount();
...
/*
public class CSaleProductDialog extends CBaseProductSelectDialog implements ActionListener, ValueChangedListener, ItemListener {
