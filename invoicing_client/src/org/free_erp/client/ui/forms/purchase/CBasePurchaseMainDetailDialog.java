 ...
 */
public abstract class CBasePurchaseMainDetailDialog extends CBaseFormDialog implements ItemListener, ValueChangedListener
{
    ...
    protected JDataNumberField payDateField;
	protected JDataField commentField;
    protected CBasePurchaseProductDialog productSelectDialog;
    protected JDataNumberField offerMoneyField;
    
	public CBasePurchaseMainDetailDialog(Frame parent, IDataSource dataSource, IDbSupport dbSupport)
    {
	...
	public CBasePurchaseMainDetailDialog(Frame parent, IDbSupport dbSupport)
    {
	...
	protected void initComps()
    {
        ...
        offerMoneyField = new JDataNumberField("offerMoney", Double.class, this.dataSource);
        offerMoneyField.setForeground(Color.red);
        offerMoneyField.setHorizontalAlignment(JLabel.RIGHT);
        offerMoneyField.setFont(font);
        offerMoneyField.addValueChangedListener(this);
        int x = 80;
        bPanel.addComponent(offerMoneyField, x, 40, 160, 25, "优惠金额", 60);
		...
		bPanel.addComponent(sumField, x + 230, 40, 160, 25, "元  合计金额", 80);
        ...
        bPanel.addComponent(chineseMoneyField, x + 480, 40, 210, 25, "大写:", 30);
	...
	protected void doCheck()
    {
		...
		if (MessageBox.showQuestionDialog(this, "您确信审核通过当前单据?") == JOptionPane.YES_OPTION)
        {
			...
			try
            {
                Main.getServiceManager().getPurchaseService().passCommonPurchaseForm(po);
                FormPo p = (FormPo)po;
                this.logContent = "审核";
	...
	protected void doDiscard()
    {
		if (MessageBox.showQuestionDialog(this, "您确信作废当前单据?") == JOptionPane.YES_OPTION)
        {
			...
			this.save();
			this.refresh();
            Object po = ((ObjectDataRow) dataRow).getUserObject();
            FormPo p = (FormPo)po;
            this.logContent = "作废";
            this.writeSystmLog(p);
		}
    ...
    public CBasePurchaseProductDialog getNewProductSelectDialog()
    {
        ...
        {
            this.productSelectDialog = new CBasePurchaseProductDialog(this, this.table.getDataSource(),null);
        }
        return this.productSelectDialog;
    }
	...
	protected void doAdd()
    {
        ...
        if (!checkSupplier())
        {
            MessageBox.showMessageDialog(this, "请先选择供应商!");
            return;
        }
		...
		CBasePurchaseProductDialog dialog = this.getNewProductSelectDialog();
	...
	protected void doEdit()
    {
		...
		CBasePurchaseProductDialog dialog = this.getNewProductSelectDialog();//new CBaseProductSelectDialog(this, this.table.getDataSource());
	...
	private void refreshTotailMoney()
    {
        ...
        String offerMoney   = offerMoneyField.getText();
        Double  offer  = 0d;
        if(offerMoney != null && !offerMoney.trim().equals(""))
        {
            offer  =Double.parseDouble(offerMoney);
            money = money - offer;
        }
...
//	private void setBindingComponentsEnable(boolean enable)
//    {
//		updateComponentsEnable(this.getContentPane(), enable);
//	}
//
//	private void updateComponentsEnable(Component comp, boolean enable)
//    {
...
//	}
    ...
    public void valueChanged(ValueChangedEvent evt)
    {
        ...
        if (evt.getSource() == this.payDateField)
        {
            String strPayDate = this.payDateField.getText();
            try
            {
            ...
            {
                this.payDateField.setText("30");
            }
        }
        if (evt.getSource() == this.offerMoneyField)
        {
            String offerMoney   = offerMoneyField.getText();
            Double  offer  = 0d;
            if(offerMoney != null && !offerMoney.trim().equals(""))
            {
                ...
                {
                    offer = Double.parseDouble(offerMoney);
                    if(offer > sumField.getMoney())
                    {
                        MessageBox.showErrorDialog(this, "优惠金额不能大于总金额！");
                        offerMoneyField.setText("0.00");
                        return;
                    }
                }
                catch(Exception ex)
                {
                    MessageBox.showErrorDialog(this, "优惠金额输入有误！");
                    offerMoneyField.setText("0.00");
                }
            }
            this.refreshTotailMoney();
    ...
    protected void beforeInitComponent()
    {
        ...
        supplierField.setRequired("供应商", true);
        ...
        payDateField = new JDataNumberField("payTerm", Integer.class, this.dataSource);
