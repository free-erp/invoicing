 ...
 */
public class CPurchaseOrderBackWindow extends CBasePurchaseListWindow
{
	public CPurchaseOrderBackWindow(String title)
	{
		super(title);
            
             this.chooseButton.setVisible(true);
        this.chooseButton.setText("选择采购单");
    ...
    private void initDatas()
	{
		...
		List<PurchaseBackPo> purchaseBackPos = purchaseService.getAllPurchaseBackPoForms(Main.getMainFrame().getCompany());
		for(PurchaseBackPo po:purchaseBackPos)
		{
    ...
    protected void doSearch()
    {
        ...
        IPurchaseService purchaseService = Main.getServiceManager().getPurchaseService();
        List<PurchaseBackPo> purchaseBackPos = purchaseService.findPurchaseBackPoForms(vo);
        if(purchaseBackPos == null || purchaseBackPos.size() == 0)
        {
            this.clearInfo();
        }
        for(PurchaseBackPo po:purchaseBackPos)
    ...
    protected void removeNumberLog()
    {
        ...
        systemLog.setContent("删除采购退货单-编号:" + this.removeNumber);
    ...
    protected CBaseFormDialog getFormDialog()
    {
        if (formDialog == null)
        {
            formDialog =  new CPurchaseOrderBackDialog(this, this.dataSource, dbSupport);
        }
        return formDialog;
    }
    @Override
    ...
    {
        return new CChoosePurchaseOrderDialog(this, this.dataSource, dbSupport);
    }
