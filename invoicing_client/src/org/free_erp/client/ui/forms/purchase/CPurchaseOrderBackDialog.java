 ...
 */
public class CPurchaseOrderBackDialog extends CBasePurchaseMainDetailDialog
{
    //临时
    protected JDataField defaultPriceField;

    public CPurchaseOrderBackDialog(Frame parent, IDataSource dataSource, IDbSupport dbSupport)
	{
		super(parent, dataSource, dbSupport);
        this.setTitle("采购退货单");
	}
    
    public CPurchaseOrderBackDialog(Frame parent, IDbSupport dbSupport)
	{
    ...
    public void newDataRow()
    {
        ...
        IDataRow dataRow = ObjectDataRow.newDataRow(po, "id", dbSupport);
...
//
//		column = new JDataTableColumn();
//		column.setHeaderText("税额");
//		column.setColumnName("priceTaxMoney");
//		column.setWidth(100);
//        column.setValueType(Currency.class);
//		columnModel.addColumn(column);
    ...
    {
        return PurchaseBackDetailPo.class;
    }

    public Customer getSupplier()
    {
        return (Customer)this.supplierField.getSelectedItem();
    }

    @Override
    public CBasePurchaseProductDialog getNewProductSelectDialog()
    {
        return new CPurchaseBackProductDialog(this, this.table.getDataSource(), this.getStorage(), this.getSupplier());
    }
