...
public class CStorageCheckWindow extends CBaseListWindow
{
	public CStorageCheckWindow(String title)
	{
		super(title);
	...
	private void initDatas()
	{
		...
		List<CheckStoragePo> checkStoragePo = storageService.getAllCheckStorageForms(Main.getMainFrame().getCompany());
		for(CheckStoragePo po:checkStoragePo)
		{
     ...
     protected CBaseFormDialog getFormDialog()
	{
        if (formDialog == null)
        {
            formDialog =  new CStorageCheckDialog(this, this.dataSource, dbSupport);
        }
        return formDialog;
	}
    ...
    protected void doSearch()
    {
        ...
        IStorageService storageService = Main.getServiceManager().getStorageService();
        List<CheckStoragePo> checkStoragePos = storageService.findCheckStorageForms(vo);
        if(checkStoragePos == null || checkStoragePos.size() == 0)
        {
            this.clearInfo();
        }
        for(CheckStoragePo po:checkStoragePos)
		{
    ...
    protected void removeNumberLog()
    {
        ...
        systemLog.setContent("删除库存盘点单-编号:" + this.removeNumber);
