 ...
 * @Changer liufei
 */
public class CStorageMoveWindow extends CBaseListWindow
{
	public CStorageMoveWindow(String title)
	{
		super(title);
	...
	protected void initTableColumns()
	{
		...
		columnModel.addColumn(column);
           column = new JDataTableColumn();
		column.setHeaderText("移出库");
		column.setColumnName("outStorage.name");
		column.setWidth(80);
		columnModel.addColumn(column);
        column = new JDataTableColumn();
		column.setHeaderText("移入库");
...
//		columnModel.addColumn(column);
	...
	private void initDatas()
	{
		...
		List<MoveStoragePo> moveStoragePo = storageService.getAllMoveStorageForms(Main.getMainFrame().getCompany());
		for(MoveStoragePo po:moveStoragePo)
		{
    ...
    protected String getExcelExportTitle()
    {
      ...
      protected void doPrint()
     {
        ...
        vo.setTitle("商品移库信息报表");
    
           Image image = Main.getMainFrame().getCompanyLogo();
        vo.setImage(image);
            variables = ReportUtilities.creatParameterMap(vo);
        CPrintDialog printDialog = new CPrintDialog(this, this.getClass().getResourceAsStream("/com/e68erp/demo/client/report/jaspers/ProductMoveReport.jasper"),variables, this.dataSource.getDataRows());
    ...
    protected CBaseFormDialog getFormDialog()
	{
        if (formDialog == null)
        {
            formDialog =  new CStorageMoveDialog(this, this.dataSource, dbSupport);
        }
        return formDialog;
	}
    @Override
    ...
    protected void removeNumberLog()
    {
        ...
        systemLog.setContent("删除移库单-编号:" + this.removeNumber);
