 ...
 * @Changer liufei
 */
public abstract class CBaseStorageMainDetailDialog extends CBaseFormDialog{
    ...
    protected CPanel bPanel;
	//protected CBaseProductSelectDialog dialog;
	public CBaseStorageMainDetailDialog(Frame parent, IDataSource dataSource, IDbSupport dbSupport)
    {
	...
	public CBaseStorageMainDetailDialog(Frame parent, IDbSupport dbSupport)
    {
	...
	protected void initComps()
    {
		...
		bPanel.addComponent(this.addButton, 750- 320, 10, 100, 22);
		bPanel.addComponent(this.modifyButton, 750 -210, 10, 100, 22);
                bPanel.addComponent(this.removeButton, 750- 100, 10, 100, 22);
		pagePane.add(bPanel);
        bPanel.setBounds(CPagePane.X, CPagePane.Y + 350, 750, 80);
    ...
    protected void initBottomPanel()
    {
		...
		bPanel.addComponent(sumField, 80, 40, 250, 25, "合计金额", 60);
        ...
        bPanel.addComponent(chineseMoneyField, 400, 40, 320, 25, "大写:人民币", 70);
    }
	...
	protected void doCheck()
    {
		...
		if (MessageBox.showQuestionDialog(this, "您确信审核通过当前单据?") == JOptionPane.YES_OPTION) {
			...
			try {
                Main.getServiceManager().getStorageService().passCommonStorageForm(po);
                FormPo p = (FormPo)po;
                this.logContent = "审核";
			...
			} catch (ProductInitializedException ex) {
				MessageBox.showMessageDialog(this, "商品已经在该仓库中存在，不能做期初\"" + ex.getProduct().getProduct().getName() + "\"!您可以选择仓库盘点来改变库存!");
    ...
    public CBaseProductSelectDialog getNewProductSelectDialog()
    {
        ...
        {
            this.productSelectDialog = new CBaseProductSelectDialog(this, this.table.getDataSource(), this.getStorage());
        }
	...
	protected void doAdd() {
        ...
        {
            MessageBox.showMessageDialog(this, "请先选择保管员!");
            return;
        }
	...
	protected void doEdit() {
		...
		CBaseProductSelectDialog dialog = this.getNewProductSelectDialog(); //new CBaseProductSelectDialog(this, this.table.getDataSource(), (Storage)this.storageField.getSelectedItem()); //
        dialog.setDataValue();
	...
	protected void doRemove() {
		...
		if (MessageBox.showQuestionDialog(this, "您确信要删除当前记录吗?") == 0) {
			...
			Set details = (Set) objRow.getColumnValue("details");
			//if (details != null) {
            //    dataRow.setColumnValue("state", -1);//afa test!
				details.remove(((ObjectDataRow) dataRow).getUserObject());
			//}
            try
            {
                this.save(); //删除的时候必须save,不然会导致dataSource与table数据不一致！afa2009-06-24
            }
            catch(Exception ex)
            {
...
//            }
    ...
    protected void beforeInitComponent() {
        ...
        storageField.addValueChangedListener(new ValueChangedListener()
        {
            
            public void valueChanged(ValueChangedEvent evt) {
                Object storage = storageField.getSelectedItem();
                if (storage instanceof Storage)
                {
                    Employee employee = ((Storage)storage).getEmployee();
                    if (employee != null)
                    {
                        adminField.setSelectedItem(employee);
                        departmentField.setText(((Employee)employee).getCatalog().getName());//liufei
                    }
                }
            }
        });
        adminField = new JDataTableComboBox("employee", Employee.class,this.dataSource, Main.getMainFrame().getObjectsPool().getEmployeeTable(), "name");
        adminField.setRequired("保管员", true);
        adminField.addValueChangedListener(new ValueChangedListener()
        {
            
            public void valueChanged(ValueChangedEvent evt) {
                Object employee = adminField.getSelectedItem();
                if (employee instanceof Employee)
                {
                    departmentField.setText(((Employee)employee).getCatalog().getName());
                }
            }
        });
