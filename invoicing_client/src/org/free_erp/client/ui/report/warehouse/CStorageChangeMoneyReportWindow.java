 ...
 */
public class CStorageChangeMoneyReportWindow extends CBaseQueryWindow implements ActionListener
{
    ...
    public CStorageChangeMoneyReportWindow(String title)
    {
        super(title);
    }
    ...
    protected CPanel getMainPanel() {
        ...
        topPanel.addComponent(numberField, x + 170, y, 120, 20, "商品", 50);
    ...
    protected void initColumns()
    {
		...
		column = new JDataTableColumn();
		column.setHeaderText("原始价格");
		column.setColumnName("oldPrice");
		column.setWidth(80);
		columnModel.addColumn(column);

        column = new JDataTableColumn();
		column.setHeaderText("现在价格");
    ...
    protected void select()
    {
        ...
        List<PriceStorageDetailPo> priceStorageDetailPos = storageService.findPriceStorageDetails(vo);
        for(PriceStorageDetailPo po:priceStorageDetailPos)
		{
			IDataRow dataRow = new ObjectDataRow(po, "id", null);
			model.insertDataRow(dataRow);
		}
        if(priceStorageDetailPos == null || priceStorageDetailPos.size() == 0)
        {
            this.clearInfo();
        }
    }
    ...
    protected void doPrint()
    {
        ...
        vo.setTitle("库存商品变价明细报表");
        Image image = Main.getMainFrame().getCompanyLogo();
        vo.setImage(image);
        variables = ReportUtilities.creatParameterMap(vo);
        CPrintDialog printDialog = new CPrintDialog(Main.getMainFrame(), this.getClass().getResourceAsStream("/com/e68erp/demo/client/report/jaspers/rStorageChangeMoney.jasper"),variables, this.dataSource.getDataRows());
