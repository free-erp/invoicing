 ...
 */
public class CStorageInListQueryWindow extends CBaseQueryWindow implements ActionListener {
    ...
    public CStorageInListQueryWindow(String title) {
        super(title);
    }
    ...
    protected void select() {
        ...
        List<InStorageDetailPo> inStorageDetailPos = storageService.findInStorageDetails(vo);
        
        for (InStorageDetailPo po : inStorageDetailPos) {
            IDataRow dataRow = new ObjectDataRow(po, "id", null);
            model.insertDataRow(dataRow);
        }
        if (inStorageDetailPos == null || inStorageDetailPos.size() == 0) {
            this.clearInfo();
        }
    }
