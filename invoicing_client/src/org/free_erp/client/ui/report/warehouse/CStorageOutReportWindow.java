...
import org.free_erp.client.ui.forms.warehouse.CStorageInDetailDialog;
 ...
 * @Changer liufei
 */
public class CStorageOutReportWindow extends CBaseQueryWindow  {
    ...
    public CStorageOutReportWindow(String title)
    {
        super(title);
        initComps();
    }
   ...
   protected void initColumns()
   {
    ...
    protected void select()
    {
        ...
        List<OutStoragePo> outStoragePos = storageService.findOutStorageForms(vo);
        
		for(OutStoragePo po:outStoragePos)
		{
			IDataRow dataRow = new ObjectDataRow(po, "id", null);
			model.insertDataRow(dataRow);
		}
        if(outStoragePos == null || outStoragePos.size() == 0)
        {
            this.clearInfo();
        }
    }
    ...
    protected void doPrint()
    {
        ...
        vo.setTitle("库存商品出库查询报表");
        Image image = Main.getMainFrame().getCompanyLogo();
        vo.setImage(image);
        variables = ReportUtilities.creatParameterMap(vo);
        CPrintDialog printDialog =new CPrintDialog(Main.getMainFrame(), this.getClass().getResourceAsStream("/com/e68erp/demo/client/report/jaspers/StorageInReport.jasper"), variables,this.dataSource.getDataRows());
    ...
    protected void doShowDetail()
    {
        ...
        CStorageInDetailDialog formDialog = new CStorageInDetailDialog(Main.getMainFrame(), this.dataSource, null);
        formDialog.setTitle("库存出库明细");
        formDialog.setPrintTitle("库存商品出库明细报表");
